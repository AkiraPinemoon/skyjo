<template>
    <div class="min-w-56 flex flex-col gap-2 p-2 border border-slate-800 rounded-lg">
        <div v-for="player of data" class="border border-slate-800 rounded-full h-16 w-full flex place-items-center gap-4 pr-5" :class="player.socketId == game?.data?.currentPlayerId ? ' border-lime-600' : ''">
